    ...
    try {
        if (key === "bet") {
            ...
            if (spinaudio) {
                ...
                if (bool) {
                    spinAuido.play().then(() => { }).catch((error: any) => {
                        console.log("Failed to autoplay audio:", error);
                    });
                } else {
                    spinAuido.pause()
...
const RouletteGame = () => {
    ...
    const [bets, setBets] = useState<{ placeId: string | number, amount: number }[]>([])
    ...
    useEffect(() => {
        if (showResult && activeTab) {
            let sumP = sumProfit + profit, sumL = lossAmount > 0 ? sumLost + lossAmount : sumLost
            setSumProfit(sumP)
            setSumLost(sumL)
    ...
    const renderPlace = (position: number, index: number) => {
        const number = isMobile ? (position + 1) + 12 * index : ((position + 1) * 3) - 13 * Math.floor((position) / 4) + 12 * index;
        ...
        return <div key={position} className={`col-span-1 flex items-center cursor-pointer select-none relative justify-center font-bold aspect-[1] rounded border-[1px] border-[#ffffff3d] ${color === 0 ? hover ? "bg-[#709cb9]" : "bg-[#2f4553]" : hover ? "bg-[#fa617b]" : "bg-[#fe2247]"} text-white`}
            onMouseEnter={() => { handleHoverPlace(number) }}
    ...
    const renderPlace2 = (index: number) => {
        ...
        return isMobile ? <>
            <div className="w-[50%] flex-col h-full text-white">
                <div className={`flex h-[50%] text-sm items-center cursor-pointer text-center justify-center relative select-none font-bold rounded border-[1px] border-[#ffffff3d] ${index === 1 ? leftId === selectHover ? "bg-[#fa617b]" : "bg-[#fe2247]" : leftId === selectHover ? `bg-[#709cb9]` : "bg-[#2f4553]"}`}
                    onMouseEnter={() => { handleHoverPlace(leftId) }}
                    onMouseLeave={() => { handleHoverPlace(null) }}
                ...
                <div className={`flex h-[50%] text-sm items-center cursor-pointer text-center justify-center relative select-none font-bold rounded border-[1px] border-[#ffffff3d] ${rightId === selectHover ? `bg-[#709cb9]` : `bg-[#2f4553]`}`}
                    onMouseLeave={() => { handleHoverPlace(null) }}
                    onMouseEnter={() => { handleHoverPlace(rightId) }}
            ...
            <div className={`w-[50%] text-sm flex h-full items-center cursor-pointer  text-white text-center justify-center relative select-none font-bold rounded border-[1px] border-[#ffffff3d] ${centerId === selectHover ? `bg-[#709cb9]` : `bg-[#2f4553]`}`}
                onMouseEnter={() => { handleHoverPlace(centerId) }}
                onMouseLeave={() => { handleHoverPlace(null) }}
            ...
            <div className={`md:col-span-2 col-span-1 flex items-center cursor-pointer justify-center relative select-none font-bold rounded border-[1px] border-[#ffffff3d] ${centerId === selectHover ? `bg-[#709cb9]` : `bg-[#2f4553]`}`}
                onMouseEnter={() => { handleHoverPlace(centerId) }}
                onMouseLeave={() => { handleHoverPlace(null) }}
                onClick={() => handlePlaceBet(centerId)}
            ...
            <div className={`col-span-1 flex items-center cursor-pointer justify-center relative select-none font-bold rounded border-[1px] border-[#ffffff3d] ${index === 1 ? leftId === selectHover ? "bg-[#fa617b]" : "bg-[#fe2247]" : leftId === selectHover ? `bg-[#709cb9]` : "bg-[#2f4553]"}`}
                onMouseEnter={() => { handleHoverPlace(leftId) }}
                onMouseLeave={() => { handleHoverPlace(null) }}
                onClick={() => handlePlaceBet(leftId)}
            ...
            <div className={`col-span-1 flex items-center cursor-pointer justify-center relative select-none font-bold rounded border-[1px] border-[#ffffff3d] ${rightId === selectHover ? `bg-[#709cb9]` : `bg-[#2f4553]`}`}
                onMouseLeave={() => { handleHoverPlace(null) }}
                onMouseEnter={() => { handleHoverPlace(rightId) }}
                onClick={() => handlePlaceBet(rightId)}
                style={{ aspectRatio: isMobile ? "1/1" : "2/1" }}>
    ...
    const renderPlacePanel = () => {
        return isMobile ?
            <div className={`flex flex-col justify-end w-full ${disabled ? "opacity-70" : "opacity-100"}`}>
                <div className="h-[7.08%] flex justify-end" >
                    <div className={`flex w-[60%] justify-center h-full aspect-[4/1] items-center select-none border-[1px] relative rounded ${(selectHover === 0 || (outcomeNumber === 0 && showResult)) ? "bg-green-200" : `bg-green-500`} text-white font-bold`}
                        onMouseLeave={() => { handleHoverPlace(null) }}
                        onMouseEnter={() => { handleHoverPlace(0) }}
                ...
                {[...Array(3)].map((_, index) => {
                    return <div key={index} className="flex flex-col h-[28.4%] w-full">
                    ...
                    <div className="flex justify-end">
                        {[...Array(3)].map((_, index) => {
                            ...
                            return <div key={index}
                                onMouseLeave={() => { handleHoverPlace(null) }}
                                onMouseEnter={() => { handleHoverPlace(id) }}
                                onClick={() => handlePlaceBet(id)}
                                className={`w-[20%] flex items-center select-none cursor-pointer relative justify-center font-bold aspect-[1] rounded ${id === selectHover ? "bg-[#709cb9]" : "bg-[#00000094]"}  border-[1px] border-[#424242] text-white`}>
                                2:1
                                <div className="absolute left-[50%] top-[50%] ">
                                    {renderChips(id)}
                                </div>
            ...
            : <div className={`flex w-[80%] space-x-1 ${disabled ? "opacity-70" : "opacity-100"}`}>
                <div className="w-[7.08%] h-full" >
                    <div className={`flex justify-center w-full cursor-pointer aspect-[1/3] items-center select-none border-[1px] relative rounded ${(selectHover === 0 || (outcomeNumber === 0 && showResult)) ? "bg-green-200" : `bg-green-500`} text-white font-bold`}
                        onMouseLeave={() => { handleHoverPlace(null) }}
                        onMouseEnter={() => { handleHoverPlace(0) }}
                        onClick={() => handlePlaceBet(0)}
                    >
                ...
                {[...Array(3)].map((_, index) => {
                    return <div key={index} className="flex flex-col w-[28.4%]  space-y-1">
                    ...
                    <div className="grid grid-cols-1 gap-1">
                        {[...Array(3)].map((_, index) => {
                            ...
                            return <div key={index}
                                ...
                                onMouseEnter={() => { handleHoverPlace(id) }}
                                onClick={() => handlePlaceBet(id)}
                                className={`col-span-1 flex items-center select-none cursor-pointer relative justify-center font-bold aspect-[1] rounded ${id === selectHover ? "bg-[#709cb9]" : "bg-[#00000094]"}  border-[1px] border-[#424242] text-white`}>
                                2:1
                                <div className="absolute left-[50%] top-[50%] ">
                                    {renderChips(id)}
                                </div>
    ...
    return <div className="flex w-full justify-center mt-5 items-center">
        <div className={`${isMobile ? "flex flex-col  items-center max-w-[400px]" : "flex  max-w-[1300px]"} bg-[#0f212e] w-full rounded-md overflow-hidden`}>
            {!isMobile && <div className={`w-[300px]  bg-[#213743] p-2`}>
                <div className="flex flex-col">
                    ...
                    <ChipButtonGroup
                        onChooseChip={(v) => { !disabled && selectChip(v as ChipIndex) }}
                        selected={chipValue}
                    ...
                    <AmountInput
                        ...
                        amount={0} />
                    {isAuto && <>
                        <BetNumberInput value={autoBetCount === Infinity ? 0 : autoBetCount} disabled={disabled} onChange={v => {
                            setAutoBetCount(v)
                        }} />
                        <StopProfitAmount disabled={disabled} Label={"Stop on Profit"} onChange={setStopPorfitA} value={stopProfitA === Infinity ? 0 : stopProfitA} Icon={<CurrencyIcon />} />
                        <StopProfitAmount disabled={disabled} Label={"Stop on Loss"} onChange={setStopLossA} value={stopLossA === Infinity ? 0 : stopLossA} Icon={<CurrencyIcon />} />
                    </>
                    }
                    <Button
                        onClick={() => {
                        ...
                        disabled={disabled && !activeTab || !!activeTab && stoppedAutobet} >
                        {activeTab === 0 ? "Bet" : autoBetting ? "Stop AutoBet" : "Start AutoBet"}
                    </Button>
                </div>
            </div>}
            <div className={`${isMobile ? "min-w-[350px] w-full" : "w-full  p-5"} mx-auto relative`}>
                <div className="flex flex-col w-full h-full">
                    {(!isMobile) && < div className="flex justify-center p-2 w-full">
                        <div className=" md:w-[30%] w-[60%] relative" style={{ aspectRatio: "1/1", background: "linear-gradient(#0c3a6d 0%, rgba(49, 93, 207, 0) 100%)", borderRadius: "50% 50% 0 0" }}>
                            <RouletteCanvas
                                outcomeNumber={outcomeNumber}
                                onAnimationEnd={() => { setShowResult(true) }} />
                            <div className={`absolute transition-opacity ${showResult ? "opacity-95" : "opacity-0"} duration-300 ease-out top-[50%] left-[50%] -translate-x-1/2 -translate-y-1/2 flex flex-col items-center justify-center w-[50%] aspect-square rounded-full bg-gradient-to-r ${wheelNumbers.find((n) => n.Number == outcomeNumber)?.Color == "Red" ? 'from-red-600 via-[#e79494] to-red-600' : wheelNumbers.find((n) => n.Number == outcomeNumber)?.Color == "Black" ? 'from-gray-800 via-gray-600 to-gray-800' : 'from-[#00ff15] via-[#92f09a] to-[#00ff15]'} shadow-lg`}>
                                <div className="text-white text-4xl font-bold">{outcomeNumber}</div>
                                <div className="text-white text-sm mt-2 uppercase">{wheelNumbers.find((n) => n.Number == outcomeNumber)?.Color}</div>
                            </div>
                        </div>
                    </div>}
                    <div className="flex w-full justify-end relative">
                        {isMobile && < div className={`${!isWheeling && !autoBetting ? "w-[35%] left-[20%] top-[20%]" : "w-[80%] left-[50%] top-[50%]"} absolute  -translate-y-1/2 -translate-x-1/2  z-20  transition-all duration-300 aspect-square `} style={{ aspectRatio: "1/1", borderRadius: "50% 50% 0 0" }}>
                            <RouletteCanvas
                                outcomeNumber={outcomeNumber}
                                onAnimationEnd={() => setShowResult(true)} />
                            <div className={`absolute transition-opacity ${(showResult && isWheeling) ? "opacity-95" : "opacity-0"} duration-300 ease-out top-[50%] left-[50%] -translate-x-1/2 -translate-y-1/2 flex flex-col items-center justify-center w-[50%] aspect-[1/1] rounded-full bg-gradient-to-r ${wheelNumbers.find((n) => n.Number == outcomeNumber)?.Color == "Red" ? 'from-red-600 via-[#e79494] to-red-600' : wheelNumbers.find((n) => n.Number == outcomeNumber)?.Color == "Black" ? 'from-gray-800 via-gray-600 to-gray-800' : 'from-[#00ff15] via-[#92f09a] to-[#00ff15]'} shadow-lg`}>
                                <div className="text-white text-4xl font-bold">{outcomeNumber}</div>
                                <div className="text-white text-sm mt-2 uppercase">{wheelNumbers.find((n) => n.Number == outcomeNumber)?.Color}</div>
                            </div>
                        </div>}
            ...
            {isMobile && <div className={`md:w-[300px] w-full bg-[#213743] p-2`}>
                <div className="flex flex-col">
                    <div className="text-white text-sm mt-3 flex items-center">
                        <div>Chip Value {chipValues[chipValue] / ratio}</div>
                        <div className="ml-2 w-4 h-4"><CurrencyIcon /></div>
...
class GameEngine {
    ...
    constructor() {
        this.timer = setInterval(() => {
            if (this.contain && this.canvas) {
                ...
                if (this.ballRadius !== this.radius * .8) {
                    this.ballRadius = this.radius * .8;
                    this.currentBallRaidus = this.radius * .8;
                }
            }
            this.animate();
        }, 1000 / 60);
    ...
    getWheelIndex(outcomeNumber: number) {
        return wheelNumbers.findIndex((n) => n.Number === outcomeNumber);
    }
    ...
    drawBall(angle: number) {
        const ctx = this.ctx;
        if (!ctx) return false;
        this.currentBallRaidus -= (this.state === 2 && this.currentBallRaidus > this.radius * .6) ? 3 : 0;
        ...
        const gradient = ctx.createRadialGradient(ballX, ballY, this.radius * 0.04 * 0.1, ballX, ballY, this.radius * 0.04);
...
export const RouletteCanvas = ({ outcomeNumber, onAnimationEnd }: { outcomeNumber: number, onAnimationEnd: () => void }) => {
...
const ResultModal = ({ visible, totalBetAmount, profitAmount, lossAmount, onClose }: { visible?: boolean, totalBetAmount: number, profitAmount: number, lossAmount: number, onClose: () => void }) => {
    return visible ? <div onClick={onClose} className="top-0 left-0 absolute w-full h-full z-40 bg-[#00000048] flex justify-center items-center">
        <div className="relative animate-zoomIn w-[60%] md:w-[30%] min-h-40 border-2 rounded-md border-[#36e95d] bg-[#0a0a0aaf] flex-col justify-around flex "
            style={{
        ...
        >
            <div className="flex text-white text-sm p-2 items-center justify-around font-bold">
                ...
                <div className="flex justify-end items-center">
                    ...
                    <div className="w-5 h-5 ml-1"><CurrencyIcon /></div>
                </div>
            </div>
            <div className="flex text-white text-sm p-2 items-center justify-around font-bold">
