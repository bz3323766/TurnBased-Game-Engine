...
const BlackjackGame = () => {
  ...
  const [seeds, setSeeds] = useState<string[]>([]); //[clientseed,serverhash,previouseClientseed,previouseServerseed]
  ...
  const getCards = async () => {
    ...
    setSeeds([
      data.clientSeed,
      data.serverSeedHash,
      data.previouseClientSeed,
      data.previouseServerSeed,
    ]);
          ...
          <div className="hidden col-span-1 p-2 min-h-[560px] bg-sider_panel shadow-[0px_0px_15px_rgba(0,0,0,0.25)] md:flex flex-col">
            ...
            {activedTab ? (
              <FairnessView
                clientSeed={seeds[0]}
                serverHash={seeds[1]}
                preClientSeed={seeds[2]}
                preServerSeed={seeds[3]}
              />
            ) : (
              <>
                <AmountInput
...
const FairnessView = ({
  ...
  clientSeed: string;
  serverHash: string;
  preClientSeed: string;
  preServerSeed: string;
}) => {
  ...
  const newDeck = useMemo(() => {
    let cards = [],
      cardPosition = 0,
      deck = createDeck();
    if (_client === "" || _server === "") return [];

    while (cardPosition < 52) {
      cards.push(getUniqueCard(deck, _client, _server, cardPosition++));
    }
    return cards;
  }, [_client, _server]);
      ...
      {active === 0 ? (
        <>
          ...
          <CustomInput
            ...
            icon={
              <button
                onClick={() => copyToClipboard(preClientSeed)}
                className="px-1 py-2 w-full "
              >
                <CopyIcon />
              </button>
          ...
          <CustomInput
            ...
            icon={
              <button
                onClick={() => copyToClipboard(preServerSeed)}
                className="px-1 py-2 w-full "
              >
                <CopyIcon />
              </button>
        ...
        <>
          ...
          <CustomInput
            disabled={true}
            value={_server === "" ? "" : buildPrivateHash(_server)}
            type="text"
